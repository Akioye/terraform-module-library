...
module "vpc" {
  ...
  single_nat_gateway = false   # one NAT per AZ in prod for HA
...
module "s3_uploads" {
  ...
  force_destroy     = false   # never allow destroy with objects in prod
...
module "rds" {
  ...
  deletion_protection   = true    # prod — never allow accidental deletion
...
module "eks" {
  ...
  endpoint_public_access = true              # lock down to VPN IP in real prod
  public_access_cidrs    = var.allowed_ips   # restrict to known IPs in prod

  node_instance_type = var.eks_node_instance_type
  node_capacity_type = "ON_DEMAND"   # no spot in prod — reliability first
  node_disk_size     = 20
  node_desired_size  = 2
  node_min_size      = 2
  node_max_size      = 4
